...
export default function HardSkillsBlock({ block }: { block: Block }) {
  ...
  const handleAddSkill = () => {
    updateBlock(block.id, {
      content: { ...block.content, skills: [...skills, { name: 'Nouveau Hard Skill', level: 80 }] }
    });
  };
  ...
  return (
    <div className={`w-full space-y-4 transition-all ${theme?.customClass || 'p-2'}`}>
      ...
      >
        <Plus size={14} /> Ajouter un hard skill
      </button>
    </div>
  );
}
